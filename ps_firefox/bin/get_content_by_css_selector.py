...
def out_file(url, css_selector, path):
    driver = lib.ff_automation()

    driver.start_firefox(url)

    title = driver.get_title()
    content_list = driver.get_content_by_css_selector(css_selector)
...
def main():
    ...
    parser.add_argument(
        'css_selector',
        help='テキストを取得する Web ページ内の要素の CSS Selector。')
    parser.add_argument(
        'path',
    ...
    out_file(args.url, args.css_selector, args.path)
